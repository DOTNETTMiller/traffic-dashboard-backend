...
export default function StateMessaging({ user, authToken }) {
  ...
  useEffect(() => {
    ...
    const match = userStateKey
      ? states.find((state) => state.stateKey.toLowerCase() === userStateKey)
      : null;
    setStateName(match?.stateName || (isAdmin ? 'Administrator' : ''));
  }, [states, userStateKey, isAdmin]);
