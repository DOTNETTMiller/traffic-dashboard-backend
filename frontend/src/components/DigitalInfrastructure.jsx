  ...
  const downloadStandardsReport = async (modelId, filename) => {
    try {
      ...
      let yPos = pdfUtils.addHeader(
        ...
        `IFC Model Analysis: ${filename}`,
        { titleColor: pdfUtils.COLORS.primary }
      );

      // Add model info section
      yPos = pdfUtils.addSectionHeading(doc, 'Model Information', yPos, 2);
      yPos = pdfUtils.addParagraph(
        doc,
        `This report analyzes the IFC model "${filename}" for compliance with ITS operational standards and provides recommendations for data gap remediation.`,
        yPos,
        { fontSize: 9, color: pdfUtils.COLORS.gray }
      );

      yPos += 5;

      // Process markdown content
      yPos = pdfUtils.processMarkdownForPDF(doc, markdownContent, yPos, {
      ...
      doc.text(
        "Generated by Matt's Experimental Sandbox Digital Infrastructure Suite",
        pdfUtils.DEFAULT_MARGINS.left,
        pageHeight - 15
      );

      // Save PDF with clean filename
      pdfUtils.savePDF(doc, `BIM-Standardization-Report-${filename}`);
    ...
    <div style={{ padding: '20px', maxWidth: '1400px', margin: '0 auto', height: '100%', overflow: 'auto' }}>
      ...
      {activeTab === 'upload' && (
        <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
          ...
          <form onSubmit={handleUpload}>
            ...
            <div style={{ padding: '12px', backgroundColor: '#eff6ff', borderRadius: '4px', marginBottom: '15px', fontSize: '14px', color: '#0369a1' }}>
                  ...
                  <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'flex-end' }}>
                      <span style={{
                        ...
                        {model.extraction_status?.toUpperCase() || 'PENDING'}
                      </span>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
      ...
      {activeTab === 'details' && modelDetails && (
        <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
          ...
          <div style={{ marginBottom: '30px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '10px' }}>
              {Object.entries(modelDetails.by_type || {}).map(([type, count]) => (
                <div key={type} style={{ padding: '12px', backgroundColor: '#f9fafb', borderRadius: '4px', display: 'flex', justifyContent: 'space-between' }}>
                  <span>{type}</span>
                  <strong>{count}</strong>
                </div>
              ))}
          ...
          <div style={{ marginBottom: '30px' }}>
            ...
            {gaps.length === 0 ? (
              <p style={{ color: '#666', padding: '20px', backgroundColor: '#f0fdf4', borderRadius: '4px' }}>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px', marginBottom: '20px' }}>
                  <div style={{ padding: '20px', backgroundColor: '#fef2f2', borderRadius: '8px', border: '2px solid #d32f2f' }}>
                    <div style={{ fontSize: '32px', fontWeight: 'bold', color: '#d32f2f', marginBottom: '8px' }}>
                      {gaps.filter(g => g.severity === 'high').length}
                    </div>
                    <div style={{ color: '#666', fontSize: '14px' }}>High Severity Gaps</div>
                  </div>
                  <div style={{ padding: '20px', backgroundColor: '#fffbeb', borderRadius: '8px', border: '2px solid #f57c00' }}>
                    <div style={{ fontSize: '32px', fontWeight: 'bold', color: '#6b7280', marginBottom: '8px' }}>
                      {gaps.filter(g => g.severity === 'medium').length}
                    </div>
                    <div style={{ color: '#666', fontSize: '14px' }}>Medium Severity Gaps</div>
                  </div>
                  <div style={{ padding: '20px', backgroundColor: '#f7fee7', borderRadius: '8px', border: '2px solid #689f38' }}>
      ...
      {activeTab === 'documentation' && (
        <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
          ...
          <div style={{ marginTop: '40px', padding: '24px', backgroundColor: '#f8faff', borderRadius: '12px', border: '1px solid #dbeafe' }}>
