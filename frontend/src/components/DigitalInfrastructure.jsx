...
function DigitalInfrastructure() {
  ...
  const downloadStandardsReport = async (modelId, filename) => {
    try {
      ...
      // Render the markdown to a PDF that matches the in-app viewers (proper
      // headings, tables, lists, links and unicode) instead of dumping raw text.
      const pdfUtils = await import('../utils/pdfExport');
      await pdfUtils.markdownToPDF(markdownContent, `BIM-Standardization-Report-${filename}`, {
        title: 'BIM Standardization Report',
        subtitle: `IFC Model Analysis: ${filename}`
      });
