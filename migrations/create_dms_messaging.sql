...
CREATE INDEX IF NOT EXISTS idx_dms_approvals_state ON dms_template_approvals(state_code, approval_status);
...
INSERT INTO dms_template_approvals (template_id, state_code, approval_status)
SELECT t.id, s.state_code, 'pending'
FROM dms_message_templates t
CROSS JOIN (
  SELECT DISTINCT state_code FROM states WHERE state_code IS NOT NULL
) s
